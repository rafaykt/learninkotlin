...
import comjetbrainshandsonkmmsharedcache.Rocket
import comjetbrainshandsonkmmsharedcache.SelectAllLaunchesInfo
import kotlin.Any
...
private class AppDatabaseQueriesImpl(
  ...
  override fun <T : Any> selectAllLaunchesInfo(mapper: (
    ...
    name: String?,
    type: String?
  ) -> T): Query<T> = Query(-250490172, selectAllLaunchesInfo, driver, "AppDatabase.sq",
  ...
  override fun insertLaunch(
    ...
    notifyQueries(-2002650468, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  override fun insertRocket(
    ...
    notifyQueries(-1818485315, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllLaunches() {
    driver.execute(157457518, """DELETE FROM Launch""", 0)
    notifyQueries(157457518, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllRockets() {
    driver.execute(-797525582, """DELETE FROM Rocket""", 0)
    notifyQueries(-797525582, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  private inner class SelectRocketByIdQuery<out T : Any>(
    ...
    override fun execute(): SqlCursor = driver.executeQuery(256868786, """
    |SELECT * FROM Rocket
    |WHERE id = ?
    """.trimMargin(), 1) {
      bindString(1, id)
